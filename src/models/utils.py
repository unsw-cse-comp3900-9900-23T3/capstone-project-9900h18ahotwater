...
def train_one_epoch(model, optimizer, data_loader, device, epoch, threshold=0.5):
    ...
    for step, data in enumerate(data_loader):
        ...
        tp += torch.sum(torch.mul(pred_class,y.to(device)))
        tn += torch.sum(torch.mul(1-pred_class,1-y.to(device)))
        fn += torch.sum(torch.mul(1-pred_class,y.to(device)))
        fp += torch.sum(torch.mul(pred_class,1-y.to(device)))
        precision = tp/(tp+fp)
        recall = tp/(tp+fn)
        ...
        data_loader.desc = "[train epoch {}] loss: {:.3f}, precision: {:.3f}, recall: {:.3f}, f1: {:.3f}".format(epoch,
...
def evaluate(model, data_loader, device, epoch, threshold=0.5):
    ...
    for step, data in enumerate(data_loader):
        ...
        data_loader.desc = "[valid epoch {}] loss: {:.3f}, precision: {:.3f}, recall: {:.3f}, f1: {:.3f}".format(epoch,
                                                                               accu_loss.item() / (step + 1),
                                                                               precision.item(),
                                                                               recall.item(),
                                                                               f1.item())
